...
const DB_NAME = "oralGradesDB_secure_v4";
const DB_VERSION = 1;
...
// Durchschnitt → Farbklasse (inkl. neutral für Ø≈4 oder keine Daten)
...
function renderSeating(){
  ...
  // Editor-Raster dynamisch ausfüllen
...
// CSV names helper
function parseNamesFromCSV(text){ return text.split(/\r?\n/).map(line=> line.split(/[;,]/).map(s=>s.trim()).filter(Boolean)[0] || "").filter(Boolean); }
